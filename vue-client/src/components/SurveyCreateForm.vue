...
export default {
  ...
  methods: {
    async submitPoll() {
      const filterdOptions = this.options.filter(word => word !== "");
      ...
      if (filterdOptions.length < 2) {
        ...
        this.loading = true;
        const currenySurvey = await postSurvey(
          new Survey(this.question, filterdOptions)
        );
        this.loading = false;
    ...
    setOption: function(option, index) {
      if (option.trim() != "") this.options[index] = option;
    },
